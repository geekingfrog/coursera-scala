...
class Empty extends TweetSet {
  ...
  override def ascendingByRetweet0(accu: Trending) = accu
...
//    (left union (right) union that) incl elem
  ...
  def filter0(p: Tweet => Boolean, accu: TweetSet): TweetSet =
    if (p(elem))
      (left union right) filter0 (p, accu incl elem)
    else
      (left union right) filter0 (p, accu)

  override def ascendingByRetweet =
...
object GoogleVsApple {
  ...
  val googleTweets: TweetSet = TweetReader.allTweets.filter(filterList(google))

  val appleTweets: TweetSet = TweetReader.allTweets.filter(filterList(apple))
  ...
  def filterList(l: List[String])(tw: Tweet): Boolean =
    if (l.isEmpty) false
    else if (tw.text.contains(l.head))
      true
    else
      filterList(l.tail)(tw)
}

object Main extends App {
//   Some help printing the results:
   println("RANKED:")
   val f = GoogleVsApple.filterList(GoogleVsApple.google)_
//   println(TweetReader.ParseTweets.getTweetData("amazondeals", TweetData.CNET) filter(f));
